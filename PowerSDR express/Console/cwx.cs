...
// Copyright (C) 2004 - 2006  FlexRadio Systems
//
...
namespace PowerSDR
{
	...
	public class CWX : System.Windows.Forms.Form
	{
		...
		private System.Windows.Forms.Label repeatdelayLabel;
		...
		private System.Windows.Forms.Label dropdelaylabel;
...
//			Debug.WriteLine("push " + data);
		...
		private void help()
		{
			...
			t += "\n";
			t += "Radio must be running w/new keyer and in a valid cw mode and frequency.\n";
	...
	// making a total of 7 elements in a word space
		...
		private void load_alpha()
		{
			...
			if (!File.Exists(sfile))	// create default morsedef.txt
			{
#if(CWX_DEBUG)
		...
		#region Misc Routines, save and restore
		...
		public void SaveSettings()
		{
			...
			a.Add("Top/"+this.Top.ToString());
			a.Add("Left/"+this.Left.ToString());
			a.Add("Width/"+this.Width.ToString());
		...
		public void RestoreSettings()
		{
			...
			foreach(Control c in temp)
			{
				...
				else if(c.GetType() == typeof(TextBoxTS))		// the control is a TextBoxTS
					textbox_list.Add(c);
			...
			ArrayList a = DB.GetVars("CWX");				// Get the saved list of controls
			a.Sort();
			...
			foreach(string s in a)				// string is in the format "name,value"
			{
				...
				{
					this.Left = Int32.Parse(val);
				}
				else if(s.StartsWith("Top"))
				{
					this.Top = Int32.Parse(val);
				}
				else if(s.StartsWith("Width"))
				{
					this.Width = Int32.Parse(val);
				}
        ...
        private void SendBufferMessage()				//CAT Read Thread
        {
            while (true)									//do forever
            {
                ...
                Thread.Sleep(2);						// give the cpu a rest
            }
        }
...
//			ttdel = 50;
		...
		private void InitializeComponent()
		{
            ...
            this.repeatdelayLabel = new System.Windows.Forms.Label();
		...
		public void s1_Click(object sender, System.EventArgs e)
		{
            if (!running)
                setup_timer();
			queue_start(1);
		...
		public void s2_Click(object sender, System.EventArgs e)
		{
            if (!running)
                setup_timer();
			queue_start(2);
		...
		public void s3_Click(object sender, System.EventArgs e)
		{
            if (!running)
                setup_timer();
			queue_start(3);
		...
		public void s4_Click(object sender, System.EventArgs e)
		{
            if (!running)
                setup_timer();
			queue_start(4);
		...
		public void s5_Click(object sender, System.EventArgs e)
		{
            if (!running)
                setup_timer();
			queue_start(5);
		...
		public void s6_Click(object sender, System.EventArgs e)
		{
            if (!running)
                setup_timer();
			queue_start(6);
		...
		// stop button clicked
		public void stopButton_Click(object sender, System.EventArgs e) // changes yt7pwr
		{
			clear_show();
			quit = true;
			kquit = true;
		...
		{
			if (File.Exists(sfile)) File.Delete(sfile);			// out withe the old
			using (StreamWriter sw = new StreamWriter(sfile))	// and in with the new
			{
				for (int i = 0; i < 64; i++)
				{
					sw.WriteLine(a2m2[i]);
