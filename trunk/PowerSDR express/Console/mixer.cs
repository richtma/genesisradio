...
namespace PowerSDR
{
	public class Mixer
	{
        ...
        private static int GetVolume_RealtekHDaudio(int mixerID, int dst_type, int src_type)
        {
            if (mixerID > mixerGetNumDevs() - 1 ||
                mixerID < 0)
                return -1;
        ...
        private static bool SetVolume_RealtekHDaudio(int mixerID, int dst_type, uint ctrl_type, int src_type, int val)
        {
            if (mixerID > mixerGetNumDevs() - 1 ||
                mixerID < 0)
                return false;
        ...
        public static bool InitRealtekHDaudioRecord(int mixerID, int record_level)
        {
            int mixer;
            int retval = -1;
            retval = mixerOpen(out mixer, mixerID, 0, 0, 0);
        ...
        public static bool InitRealtekHDaudioPlay(int mixerID)
        {
            int mixer;
            int retval = -1;
            retval = mixerOpen(out mixer, mixerID, 0, 0, 0);
            ...
            bool success = GetControlByType(mixer, MIXERLINE_COMPONENTTYPE_DST_SPEAKERS, 4104, MIXERCONTROL_CONTROLTYPE_MUX, out ctrl);
            if (!success)
                goto failed;
