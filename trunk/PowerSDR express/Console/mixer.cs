...
namespace PowerSDR
{
	public class Mixer
	{
            ...
            for (int j = 0; j < num_dest; j++)	// for each destination line
            {
                ...
                if (retval == MMSYSERR_NOERROR && dst_line.dwComponentType == dst_type)
                {
                    if (src_type == 0)
                    {
                        ...
                        // Get the control 
                        retval = mixerGetLineControls(mixer, ref dst_lc, MIXER_GETLINECONTROLSF_ALL);
                        if (retval == MMSYSERR_NOERROR)
                        {
                            MIXERCONTROL ctrl = new MIXERCONTROL();
                            ctrl.cbStruct = mcSize;
                    ...
                    {                                               // Realtek HD audio
                        ...
                        for (int k = 0; k < num_src; k++)	// for all source lines connected to this destination
                        {
                            ...
                            src_line.dwSource = k;
                            retval = mixerGetLineInfo(mixer, ref src_line, MIXER_GETLINEINFOF_SOURCE);
                            if (retval == MMSYSERR_NOERROR && src_line.szPname.Contains("Realtek HD Audio output"))
                            {
                                MIXERLINECONTROLS src_lc = new MIXERLINECONTROLS();
                                int mcSize = 152;
                                src_lc.pamxctrl = Marshal.AllocCoTaskMem(mcSize);
                                ...
                                // Get the control 
                                retval = mixerGetLineControls(mixer, ref src_lc, MIXER_GETLINECONTROLSF_ALL);
                                if (retval == MMSYSERR_NOERROR)
                                {
                                    MIXERCONTROL ctrl = new MIXERCONTROL();
                                    ctrl.cbStruct = mcSize;
        ...
        private static int GetVolume_NoMixerAudio(int mixerID, int dst_type, int src_type)
        {
            if (mixerID > mixerGetNumDevs() - 1 ||
                mixerID < 0)
                return -1;
        ...
        private static bool SetVolume_NoMixerAudio(int mixerID, int dst_type, uint ctrl_type, int src_type, int val)        // yt7pwr
        {
            if (mixerID > mixerGetNumDevs() - 1 ||
                mixerID < 0)
                return false;
            ...
            int mixer;
            MIXERCONTROL volCtrl = new MIXERCONTROL();
            int currentVol = GetVolume_RealtekHDaudio(mixerID, src_type, dst_type);
            if (currentVol == val) return true;
            ...
            bool success = GetControlByID(mixer, src_type, dst_type, MIXERCONTROL_CONTROLTYPE_VOLUME, out volCtrl);
            if (success == true)
            {
                if (val > volCtrl.lMaximum) val = volCtrl.lMaximum;
                if (val < volCtrl.lMinimum) val = volCtrl.lMinimum;
                ...
                if (retval == MMSYSERR_NOERROR)
                {
                    currentVol = GetVolume_RealtekHDaudio(mixerID, dst_type, src_type);
                    if (currentVol != val)
                        success = false;
                    else success = true;
                }
        ...
        private static bool SetVolume_RealtekHDaudio(int mixerID, int dst_type, uint ctrl_type, int src_type, int val)      // yt7pwr
        {
            if (mixerID > mixerGetNumDevs() - 1 ||
                mixerID < 0)
                return false;
        ...
        private static bool SetMute_RealtekHDaudio(int mixerID, uint dst_type, uint src_type, bool val)
        {
            if (mixerID > mixerGetNumDevs() - 1 ||
                mixerID < 0)
                return false;
            ...
            bool success = GetControlByType(mixer, dst_type, src_type, MIXERCONTROL_CONTROLTYPE_MUX, out muteCtrl);
            if (success == true)
            {
        ...
        public static bool SetLineInRecordVolume_NoMixerAudio(int mixerID, int percent)     // yt7pwr
        {
            try
            {
                int mixer;
                ...
                if (success)
                {
                    if (ctrl.szName.Contains("Master") || ctrl.szName.Contains("Rec"))
                    {
                        return SetVolume_NoMixerAudio(mixerID, -1, MIXERCONTROL_CONTROLTYPE_VOLUME, 0,
                            UInt16.MaxValue * percent / 100);
                    }
                    return false;
                }
        ...
        public static bool SetLineInRecordVolume_RealtekHDaudio(int mixerID, int percent)
        {
            try
            {
                int mixer;
                ...
                if (success)
                {
                    ...
                    {
                        return SetVolume_RealtekHDaudio(mixerID, -1, MIXERCONTROL_CONTROLTYPE_VOLUME, 0,
                            UInt16.MaxValue * percent / 100);
                    }
                    return false;
                }
        ...
        public static bool SetMainMute_RealtekHDaudio(int mixerID, bool val)
        {
            return SetMute_RealtekHDaudio(mixerID, MIXERLINE_COMPONENTTYPE_DST_SPEAKERS, 4104, val);
        }
...
/*        public static int GetMux(int mixerID)
        ...
        public static bool SetMux_RealtekHDaudio(int mixerID, int index, bool tx)
        {
            if (mixerID > mixerGetNumDevs() - 1 ||
                mixerID < 0)
                return false;
        ...
        public static bool InitNoMixerAudioRecord(int mixerID, int record_level)      // yt7pwr
        {
            try
            {
                int mixer;
        ...
        public static bool InitNoMixerAudioPlay(int mixerID)      // yt7pwr
        {
            try
            {
                int mixer;
